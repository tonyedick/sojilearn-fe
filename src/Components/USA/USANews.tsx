...
export default function USANews() {
    const [usaPosts, setUSAPosts] = useState<BlogPost[]>([]);
    ...
    const fetchUSAPosts = async () => {
        ...
        setUSAPosts((data as any[]) || []);
        } catch (error) {
        console.error('Error fetching USA posts:', error);
        } finally {
        setLoading(false);
        }
    };
